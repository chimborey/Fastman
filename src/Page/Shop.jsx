...
const Shop = () => {
      const { fillterProducts, setSearch, addToCart, toggleLike, likeCart } = useContext(ShopContext)
      return (
                  ...
                  <main className=' mt-12 container'>
                        <div className=' flex justify-between items-center'>
                              <input
                                    ...
                                    className='w-full overflow-hidden py-2 px-2 text-gray-500 border-2 focus:outline-none bg-white rounded-lg' />
                        </div>
                                    ...
                                    fillterProducts.map((item) => {
                                          ...
                                          return (
                                                <div data-aos="fade-up" data-aos-delay='600'>
                                                      <div key={item.id} className='w-full relative overflow-hidden border border-black rounded-lg bg-white'>
                                                            ...
                                                            {/* Add to Cart Button */}
                                                            <Link to={`/shop/${item.id}`} onClick={() => addToCart(item)} className='mt-1 cursor-pointer absolute top-2 right-3 flex items-center justify-center px-2 py-1 bg-green-500 text-white text-sm hover:bg-green-600 transition-all rounded-tl-full  rounded-br-full'>
                                                                  add
